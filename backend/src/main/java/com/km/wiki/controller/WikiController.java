...
public class WikiController {
     ...
     */
    @PostMapping
    public Result<WikiVO> create(@RequestBody WikiDTO.CreateRequest request) {
        // 获取当前用户ID
     ...
     */
    @PutMapping("/{id}")
    public Result<WikiVO> update(@PathVariable Long id, @RequestBody WikiDTO.UpdateRequest request) {
    ...
    public Result<List<WikiMemberVO>> getMembers(@PathVariable Long id) {
        List<WikiMemberVO> members = wikiService.getWikiMembers(id);
    ...
    public Result<Void> addMember(@PathVariable Long id, @RequestBody WikiDTO.AddMemberRequest request) {
        wikiService.addWikiMember(id, request.getUserId(), request.getRoleInWiki());
        return Result.success();
    }
